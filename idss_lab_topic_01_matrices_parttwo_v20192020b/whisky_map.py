...
def KL_2d(i, j):
    KL_2d = 0.5*(
        np.trace(np.linalg.inv(np.cov(
            P[region_indexs[j]], rowvar=False)*np.cov(P[region_indexs[i]], rowvar=False)))
        +
    )
...
def map_box(ax, x1, y1, x2, y2):
    ...
    ax.plot([tl[0], br[0], br[0], tl[0], tl[0]], [
            tl[1], tl[1], br[1], br[1], tl[1]], c='r', ls=':')
    ax.add_patch(Polygon(
        np.array([[tl[0], br[0], br[0], tl[0], tl[0]], [
                 tl[1], tl[1], br[1], br[1], tl[1]]]).T,
...
def draw_map(locations, distilleries, map_attribute=None):
    ...
    if map_attribute is not None:
        ...
        img = ax.imshow(gridded.T, extent=(-7.8, -0.5,
                                           54.0, 61.0), origin='top')
    ...
    for i in range(7, len(distilleries), 10):
        ax.text(transformed[i, 0], transformed[i, 1],
                distilleries[i], fontsize=8)
